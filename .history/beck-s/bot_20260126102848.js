...
// ================== CONFIG ==================
const BOT_TOKEN = "8010354722:AAGF5owYJT4Dp1pt-3uNIwsQGniOfDi1ilo";
const ADMIN_GROUP_ID = -1003570889560; // admin guruh ID
...
async function getImageSource(imgPath) {
  ...
  if (fs.existsSync(file)) {
    return {
      ...
      contentType: getImageContentType(imgPath),
    };
  }
  return null;
}
...
// ================== SEND-CART ==================
async function sendCart(userId, products) {
  const usdRate = await getUSDRate();
  const { text, totalUSD } = formatOrderText(products);
  const totalUZS = totalUSD * usdRate;
  ...
  for (const p of products) {
    ...
    if (img) {
      await bot.sendPhoto(userId, img, { caption, parse_mode: "HTML" });
    } else {
      await bot.sendMessage(userId, caption, { parse_mode: "HTML" });
    }
  }
...
app.post("/send-cart", async (req, res) => {
  try {
    const { telegramId, products } = req.body;
    await sendCart(Number(telegramId), products);
    res.json({ success: true });
  } catch (e) {
    console.error("❌ send-cart error:", e.message);
    res.json({ success: false });
