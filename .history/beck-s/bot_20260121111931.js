...
//BUNDA IMG HAM YUBORILITTI FAQAT OZGINA MUAMMO BOR
// import express from "express";
// import bodyParser from "body-parser";
// import cors from "cors";
// import TelegramBot from "node-telegram-bot-api";
...
━━━━━━━━━━━━━━━━━━━━`;
    ...
    // USERGA mahsulot rasmlari bilan yuborish
    ...
    for (const [index, product] of products.entries()) {
      ...
      try {
        const caption = `📦 <b>${product.title}</b>\n💰 Narxi: ${product.price}import express from "express";
import bodyParser from "body-parser";
import cors from "cors";
import TelegramBot from "node-telegram-bot-api";
import path from "path";
...
;
        ...
        console.error(`User rasm error (${product.title}):`, userImgError.message);
        // Xato bo'lsa text yuborish
        await bot.sendMessage(userId, `📦 ${product.title}\n💰 ${product.price}import express from "express";
import bodyParser from "body-parser";
import cors from "cors";
import TelegramBot from "node-telegram-bot-api";
import path from "path";
...
, {
    ...
    // ADMINGA mahsulot rasmlari bilan yuborish
    ...
    for (const [index, product] of products.entries()) {
      ...
      try {
        ...
        if (imageSource && imageSource.type === "file") {
          ...
          console.log(`📤 URL: ${imageSource.source}`);
          await bot.sendPhoto(ADMIN_GROUP_ID, imageSource.source, {
            caption,
            parse_mode: "HTML",
            message_thread_id: threadId,
        ...
        console.error(`Admin rasm error (${product.title}):`, adminImgError.message);
        await bot.sendMessage(ADMIN_GROUP_ID,
          `📦 <b>${product.title}</b>\n💰 ${product.price}$\n\n❌ Rasm yuborilmadi`,
          {
            parse_mode: "HTML",
