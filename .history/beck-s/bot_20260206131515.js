...
bot.onText(/\/help/, async (msg) => {
  try {
    if (msg.chat.type === "private") {
      const text = `📚 <b>Yordam (Foydalanuvchi)</b>`;
      return bot.sendMessage(msg.chat.id, text, { parse_mode: "HTML" });
    }

    if (msg.chat.id === ADMIN_GROUP_ID) {
      const text = `🛠 <b>Admin uchun yordam</b>`;
      return bot.sendMessage(msg.chat.id, text, {
        parse_mode: "HTML",
        message_thread_id: msg.message_thread_id,
      });
...
bot.onText(/\/userinfo/, async (msg) => {
  try {
    ...
    const text = `👤 <b>User ma'lumotlari</b>
Ismi: ${user.first_name || "-"}
Username: ${user.username ? "@" + user.username : "-"}
Telegram ID: <code>${userId}</code>`;
...
bot.on("message", async (msg) => {
  try {
    ...
    if (
      ...
      !msg.from?.is_bot   // ✅ FAQAT SHU QATOR QO‘SHILDI
    ) {
      const userId = reverseSessions[msg.message_thread_id];
      if (!userId) return;
      ...
      if (msg.photo)
        await bot.sendPhoto(userId, msg.photo.at(-1).file_id);
...
app.post("/send-cart", async (req, res) => {
  try {
    ...
    for (const p of products) {
      ...
      if (img) {
        await bot.sendPhoto(userId, img.source, { caption });
        await bot.sendPhoto(ADMIN_GROUP_ID, img.source, {
          caption,
          message_thread_id: threadId,
        });
    ...
    await bot.sendMessage(userId, "✅ Buyurtma qabul qilindi");
    return res.json({ success: true });
  } catch (e) {
    console.error("❌ send-cart error:", e.message);
    return res.json({ success: false });
