...
// Backend server (LOCAL yoki PUBLIC URL)
const BACKEND_URL = process.env.BACKEND_URL || "http://localhost:707";

// Proxy sozlamalari (agar Telegram bloklangan bo'lsa)
...
// Agar proxy kerak bo'lsa
if (USE_PROXY) {
  const HttpsProxyAgent = require("https-proxy-agent");
  botOptions.request = {
    agent: new HttpsProxyAgent(`http://${PROXY_HOST}:${PROXY_PORT}`),
  };
  console.log(`🔒 Proxy orqali ulanilmoqda: ${PROXY_HOST}:${PROXY_PORT}`);
}
 ...
 */
function getImageSource(imgPath) {
  ...
  // Agar relative path bo'lsa, to'liq URL yasash
  if (imgPath.startsWith("/uploads/") || imgPath.startsWith("uploads/")) {
    const cleanPath = imgPath.startsWith("/") ? imgPath : `/${imgPath}`;
    return { type: "url", source: `${BACKEND_URL}${cleanPath}` };
  }

  // Agar local file path bo'lsa
  const localPath = path.join(__dirname, "uploads", path.basename(imgPath));
...
━━━━━━━━━━━━━━━━━━━━`;
    ...
    // Mahsulot rasmlarini yuborish
    const imagePromises = products.map(async (product, index) => {
      const imageSource = getImageSource(product.img);
      ...
      if (imageSource) {
        try {
          ...
          if (imageSource.type === "url") {
            ...
            // Local file orqali yuborish
            await bot.sendPhoto(
              ADMIN_GROUP_ID,
              fs.createReadStream(imageSource.source),
              {
                caption,
                parse_mode: "HTML",
                message_thread_id: threadId,
