...
bot.on("message", async (msg) => {
  try {
    ...
    if (msg.chat.type === "private") {
      ...
      if (msg.voice) {
        await bot.sendVoice(ADMIN_GROUP_ID, msg.voice.file_id, options);
      }

      if (msg.video) {
        await bot.sendVideo(ADMIN_GROUP_ID, msg.video.file_id, options);
      }
      ...
      if (msg.document) {
        await bot.sendDocument(ADMIN_GROUP_ID, msg.document.file_id, options);
      }

      if (msg.photo) {
        const photo = msg.photo[msg.photo.length - 1];
        await bot.sendPhoto(ADMIN_GROUP_ID, photo.file_id, options);
      }
    }
