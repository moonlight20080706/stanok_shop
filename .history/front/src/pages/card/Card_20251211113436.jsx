    ...
    <Link to={`/product/${id}`} className="card-link">
      <article className="product-card">
        ...
        <div className="price-card">
          {price !== undefined && (
            <button className="pro-price">{formattedPrice}</button>
          )}
        </div>
        ...
        <div className="pro-card-body">
          ...
          {safeDesc && (
            ...
            //   {/* agar expanded true bo'lsa butun matn ko'rinsin, aks holda qisqargani */}
  ...
  {isLongDesc && (
    ...
    >
      {expanded ? " yopish" : " batafsil"}
       ...
       <footer className="pro-card-foot">
  {formattedDate && <time className="pro-card-date">{formattedDate}</time>}
 {!token && (
   cartItems.some(item => item.id === id) ? (
    <button className="pro-card-btn in-cart" disabled>Savatda</button>
  ) : (
    <button
      onClick={(e) => {
        e.preventDefault();
