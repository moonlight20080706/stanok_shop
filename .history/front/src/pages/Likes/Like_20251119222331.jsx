// import { useContext } from "react";
// import { Context } from "../../context/useContext";
// import { BASE_URL_IMG } from "../../service/api"; // 🔹 qo‘shildi
// import "./like.css";

// const Like = () => {
//   const { likes, removeLike } = useContext(Context);

//   if (likes.length === 0) return <p>Hech qanday yoqtirilgan mahsulot yo‘q</p>;

//   console.log(likes);

//   const getImgSrc = (img) => {
//     if (!img) return "/no-image.png";
//     if (img.startsWith("http") || img.startsWith("blob:")) return img;
//     return BASE_URL_IMG + img; // 🔹 asosiy yechim
//   };

//   return (
//     <div className="like-page">
//       <h1>Yoqtirilgan mahsulotlar</h1>
//       {likes.map((item) => (
//         <div key={item.id} className="like-item">
//           <img src={getImgSrc(item.img)} alt={item.title} width={100} />
//           <p>{item.title}</p>
//           <p>Narxi: {item.price}</p>
//           <button onClick={() => removeLike(item.id)}>O‘chirish</button>
//         </div>
//       ))}
//     </div>
//   );
// };

// export default Like;



import { useContext } from "react";
import { Context } from "../../context/useContext";
import { BASE_URL_IMG } from "../../service/api";
import "./like.css";

const Like = () => {
  const { likes, removeLike } = useContext(Context);

  if (likes.length === 0)
    return <p className="empty-like">Hech qanday yoqtirilgan mahsulot yo‘q</p>;

  const getImgSrc = (img) => {
    if (!img) return "/no-image.png";
    if (img.startsWith("http") || img.startsWith("blob:")) return img;
    return BASE_URL_IMG + img;
  };

  return (
    <div className="like-page">
      <h1>Yoqtirilgan mahsulotlar</h1>

      {likes.map((item) => (
        <div key={item.id} className="like-item">
          <img src={getImgSrc(item.img)} alt={item.title} />

          <p>{item.title}</p>
          <p>Narxi: {item.price} so'm</p>

          <button onClick={() => removeLike(item.id)}>O‘chirish</button>
        </div>
      ))}
    </div>
  );
};

export default Like;
