...
const Admin = () => {
  ...
  //       swalError("Xatolik", err.response?.data?.message || "Noto'g'ri so'rov");
  //     }
  //   } finally {
  //     setLoading(false);
  //   }
...
const onSubmitProduct = async (e) => {
  ...
  try {
    if (isEdit && editId) {
      ...
      const { data } = await api.post(`/createCard`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      // ===== BACKEND RESPONSE MOSLASHISH =====
      const created = data?.data ?? data?.card ?? data;
    ...
    if (err.response) {
      ...
      if (status === 400) {
        swalError("Noto‘g‘ri ma’lumot ❌", message);
      } else if (status === 401) {
        swalError("Ruxsat yo‘q 🔐", "Token yaroqsiz yoki muddati tugagan");
      } else if (status === 403) {
        swalError("Ta’qiqlangan ⛔", "Bu amal uchun ruxsat yo‘q");
      } else if (status === 404) {
        swalError("Topilmadi ❓", "API manzili noto‘g‘ri");
      } else if (status === 500) {
        swalError("Server xatosi 💥", "Backend ichki xatolik berdi");
      } else {
        swalError("Xatolik", message);
      }
    }
    ...
    // ===== SERVERGA ULANMAGAN =====
      ...
      swalError(
        "Ulanish xatosi 🌐",
        "Server ishlamayapti yoki internet yo‘q"
      );
    }

    // ===== BOSHQA XATOLAR =====
    else {
      swalError("Xatolik", err.message || "Noma’lum xato");
    }
  } finally {
    setLoading(false);
  }
            ...
            <form onSubmit={onSubmitSwipper}>
              ...
              <div>
                ...
                {swipperPreview ? (
                  ...
                  <button className="admin-btns" type="button" onClick={openSwipperPicker}>
                    Rasm tanlash
                  </button>
                )}
                {swipperImg && (
      ...
      {showAdmin && (
        <div className="admin-controller">
          ...
          <form onSubmit={onSubmitAdmin}>
            <FaX onClick={() => setShowAdmin(false)} className="close-icon" />
            ...
            {admins && admins.length > 0 ? (
              admins.map((a) => (
                <li key={a.id} className="admin-item">
                  ...
                  <button onClick={() => deleteAdmin(a.id)} className="admin-btns">O‘chirish</button>
                </li>
              ))
            ) : (
            ...
            {footerInfo.length > 0 ? (
              footerInfo.map((info) => (
                <div key={info.id} className="footer-item">
                  ...
                  <p>Ism: {info.name}</p>
                  <button onClick={() => editFooter(info)} className="admin-btns">✏️ Edit</button>
                  <button onClick={() => deleteFooter(info.id)} className="admin-btns">
                    🗑️ Delete
                  </button>
                </div>
              ))
              ...
              infos.map((i) => (
                <div key={i.id} className="admin-info-item">
                  ...
                  <div className="info-actions">
                    <button onClick={() => editInfo(i)} className="admin-btns">✏️ Edit</button>
                    <button onClick={() => deleteInfo(i.id)} className="admin-btns">🗑️ Delete</button>
                  </div>
                </div>
              ))
            ) : (
