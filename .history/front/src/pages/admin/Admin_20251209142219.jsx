  ...
  const onSubmitProduct = async (e) => {
    ...
    try {
      if (isEdit && editId) {
        ...
        setAdmin((prev) => {
          ...
          return {
            ...
            card: prev.card.map((c) =>
              c.id === editId
                ? { ...c, title, desc, price, quantity, category_id, status }
                : c
            ),
          };
        });
        ...
        formData.append("card_status", card_status);
        if (img) formData.append("img", img);

        const { data, status: httpStatus } = await api.post(`/createCard`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
            ...
            <form onSubmit={onSubmitSwipper}>
              ...
              <div>
                ...
                {swipperPreview ? (
                  ...
                  <button type="button" onClick={openSwipperPicker}                 className="admin-btns"
>
                    Rasm tanlash
                  </button>
                )}
            ...
            {footerInfo.length > 0 ? (
              footerInfo.map((info) => (
                <div key={info.id} className="footer-item">
                  ...
                  <p>Ism: {info.name}</p>
                  <button onClick={() => editFooter(info)} className="edit-btn">✏️ Edit</button>
                  <button onClick={() => deleteFooter(info.id)} className="delete-btn">
                    🗑️ Delete
                  </button>
                </div>
              ))
            ...
            {infos.length > 0 ? (
              infos.map((i) => (
                <div key={i.id} className="admin-info-item">
                  ...
                  <div className="info-actions admin-controller-btns">
                    <button onClick={() => editInfo(i)} className="edit-btn">✏️ Edit</button>
                    <button onClick={() => deleteInfo(i.id)} className="delete-btn">🗑️ Delete</button>
                  </div>
                </div>
              ))
            ) : (
