...
const Cart = () => {
  ...
  const totals = cartItems.reduce(
    (acc, item) => {
      const currency = item.currency || "USD"; // default $
  ...
  return (
    <section className="like-page">
          ...
          <article key={item.id} className="like-card">
            <Link to={`/product/${item.id}`} className="like-card-link">
              ...
              <div className="like-card-body">
                <h3>{item.title}</h3>
                <p className="price">
                  Boshlang'ich narx: {item.price} {item.currency || "USD"}
                </p>
      ...
      <div className="cart-summary">
        <div className="total-price">
          <h2>Jami:</h2>
          <h3>💵 $: {totals.totalUSD.toFixed(2)}</h3>
          <h3>💰 So'm: {totals.totalSom.toLocaleString()}</h3>
        </div>
