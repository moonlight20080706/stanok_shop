...
const Cart = () => {
  const { cartItems, removeFromCart, increaseCartQuantity, decreaseCartQuantity } = useContext(Context);
  const [telegramId, setTelegramId] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const getImageUrl = (img) => {
    if (!img) return "/no-image.png";
    if (img.startsWith("http") || img.startsWith("blob:")) return img;
    const cleanBase = BASE_URL_IMG.endsWith("/") ? BASE_URL_IMG.slice(0, -1) : BASE_URL_IMG;
    const cleanImg = img.startsWith("/") ? img : `/${img}`;
    return `${cleanBase}${cleanImg}`;
  };

  // ✅ JAMI HISOBLASH (Backend bilan bir xil mantiq)
  // ❌ ESKI KOD O'CHIRILDI - YANGI KOD:
  const [totals, setTotals] = useState({ totalUSD: 0, totalSom: 0 });
  const [usdRate, setUsdRate] = useState(0);

  // ✅ Kursni olish va summa hisoblash
  useState(() => {
    const calculateTotals = async () => {
      try {
        // Backend bilan bir xil kurs olish
        ...
        // Backend bilan bir xil mantiq
        let totalUSD = 0;
        let totalSom = 0;

        cartItems.forEach((item) => {
          const sum = item.price * item.cartQuantity;

          if (item.currency === "$" || item.currency === "USD") {
            totalUSD += sum;
            totalSom += sum * rate;
          } else if (item.currency === "UZS" || item.currency === "so'm") {
            totalSom += sum;
            totalUSD += sum / rate;
          }
        });

        setTotals({ totalUSD, totalSom });
      } catch (err) {
        console.error("Kurs olishda xatolik:", err);
      }
    };

    calculateTotals();
  }, [cartItems]); // cartItems o'zgarganda qayta hisoblash
  ...
  if (cartItems.length === 0) {
    return (
      <div className="empty-cart">
        ...
        <p>Hali mahsulot qo'shilmagan</p>
        <Link to="/products">
          <button>Mahsulotlarga o'tish</button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="cart-container">
      ...
      <div className="cart-summary">
        <h3>Jami:</h3>
        <p>💵 $ da: {totals.totalUSD.toFixed(2)} $</p>
        <p>💰 So'mda: {Math.round(totals.totalSom).toLocaleString()} so'm</p>
        {usdRate > 0 && <p className="exchange-rate">📊 Kurs: 1$ = {usdRate} so'm</p>}
      </div>
      ...
      <div className="telegram-form">
        ...
        <input
          ...
          placeholder="Masalan: 123456789"
        />
        <button onClick={sendCart} disabled={loading}>
          {loading ? "⏳ Yuborilmoqda..." : "✉️ Adminga yuborish"}
        </button>
      </div>
    </div>
