...
const Cart = () => {
  ...
  // ✅ TOTAL PRICE (har bir valyuta bo‘yicha)
  const totals = cartItems.reduce(
    (acc, item) => {
      if (item.currency === "$") {
        acc.usd += item.price * item.cartQuantity;
      } else if (item.currency === "so'm") {
        acc.som += item.price * item.cartQuantity;
      }
      return acc;
    },
    { usd: 0, som: 0 }
  ...
  const sendCart = async () => {
    ...
    try {
      ...
      if (data.success) {
        alert(
          `✅ Buyurtma yuborildi!\n\nJami: ${totals.usd.toFixed(
            2
          )}$ / ${totals.som.toLocaleString()} so'm`
        );
        setTelegramId("");
      } else {
        alert(data.message || "❌ Xatolik yuz berdi!");
    ...
    <section className="like-page">
      ...
      <div className="cart-summary">
        <div className="total-price">
          <h2>Jami:</h2>
          <h2 className="price">
            {totals.usd.toFixed(2)}$ / {totals.som.toLocaleString()} so'm
          </h2>
        </div>
