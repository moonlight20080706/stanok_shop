...
const Cart = () => {
  ...
  // ✅ TOTAL har ikki valyuta uchun (display uchun)
  const totals = cartItems.reduce(
    (acc, item) => {
      ...
      if (currency === "$" || currency === "USD") {
        acc.totalUSD += totalPrice;
        acc.totalSom += totalPrice * EXCHANGE_RATE;
      } else if (currency === "UZS") {
        acc.totalSom += totalPrice;
        acc.totalUSD += totalPrice / EXCHANGE_RATE;
      }
      return acc;
    },
    { totalSom: 0, totalUSD: 0 }
  ...
  const sendCart = async () => {
    ...
    // 🔹 Har product o‘z valyutasida yuboriladi
    const products = cartItems.map((item) => ({
      title: item.title,
      price: item.price,
      quantity: item.cartQuantity,
    ...
    try {
      ...
      if (data.success) {
        ...
        // 🔹 Har product o‘z valyutasida
        products.forEach((p) => {
          message += `${p.title} — ${p.quantity} × ${p.price} ${p.currency} = ${p.total} ${p.currency}\n`;
        });

        message += `\nJami:\n${
          totals.totalUSD.toFixed(2)
        } $ (jami $dagi qiymat)\n${totals.totalSom.toLocaleString()} so'm (jami so'mdagi qiymat)`;
          ...
          <article key={item.id} className="like-card">
            <Link to={`/product/${item.id}`} className="like-card-link">
              ...
              <div className="like-card-body">
                ...
                <p
                  className={
                    (item.price.toString() + (item.currency || "$")).length > 24
                      ? "price-2"
                      : "price"
                  }
                >
                  Boshlang'ich narx: {item.price}{" "}
                  {item.currency === "UZS" ? "so'm" : "$"}
                </p>
